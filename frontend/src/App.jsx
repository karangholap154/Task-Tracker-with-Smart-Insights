...
import { CheckCircle2, Circle, Plus, TrendingUp, Clock, AlertCircle, X, Trash2, Filter, Layout } from "lucide-react";
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-indigo-950 via-purple-950 to-slate-950 relative overflow-x-hidden">
      ...
      <div className="fixed inset-0 overflow-hidden pointer-events-none">
        ...
        <div className="absolute -bottom-40 -right-40 w-80 h-80 bg-pink-600 rounded-full mix-blend-multiply filter blur-3xl opacity-10 animate-pulse" style={{animationDelay: '2s'}}></div>
        <div className="absolute top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2 w-96 h-96 bg-indigo-600 rounded-full mix-blend-multiply filter blur-3xl opacity-10 animate-pulse" style={{animationDelay: '4s'}}></div>
      </div>
      ...
      <div className="relative">
            ...
            <div className="flex items-center justify-between h-16 sm:h-20">
              ...
              >
                ...
                <div className="relative bg-gradient-to-r from-purple-500 to-pink-500 text-white px-4 py-2 sm:px-6 sm:py-2.5 rounded-xl font-semibold flex items-center gap-2 hover:scale-105 active:scale-95 transition-transform shadow-lg text-sm sm:text-base">
                  {isFormOpen ? <X className="w-4 h-4 sm:w-5 sm:h-5" /> : <Plus className="w-4 h-4 sm:w-5 sm:h-5" />}
                  <span className="hidden sm:inline">{isFormOpen ? 'Close' : 'New Task'}</span>
                </div>
              </button>
            </div>
          </div>
        ...
        <main className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-6 sm:py-8 lg:py-10">
          ...
          <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 lg:gap-8">
            ...
            <div className={`lg:col-span-4 order-2 lg:order-1 ${isFormOpen ? 'block' : 'hidden lg:block'}`}>
                ...
                <div className="relative group">
                  ...
                  <div className="relative bg-slate-900/60 backdrop-blur-xl rounded-2xl border border-white/10 p-5 sm:p-6">
                    ...
                    <form onSubmit={addTask} className="space-y-4">
                      <div>
                        <label className="block text-xs font-semibold text-purple-300 mb-2 uppercase tracking-wide">Title</label>
                        <input
                          type="text"
                      ...
                      <div>
                        <label className="block text-xs font-semibold text-purple-300 mb-2 uppercase tracking-wide">Description</label>
                        <textarea
                      ...
                      <div className="grid grid-cols-2 gap-3">
                        <div>
                          <label className="block text-xs font-semibold text-purple-300 mb-2 uppercase tracking-wide">Priority</label>
                          <select
                        ...
                        <div>
                          <label className="block text-xs font-semibold text-purple-300 mb-2 uppercase tracking-wide">Due Date</label>
                          <input
                            type="date"
                            className="w-full px-3 py-3 bg-slate-800/50 border border-white/10 rounded-xl text-white focus:ring-2 focus:ring-purple-500/50 focus:border-purple-500/50 transition-all outline-none text-sm"
                            value={form.due_date}
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 sm:gap-5">
                ...
                <div className="relative group">
                  ...
                  <div className="relative bg-slate-900/60 backdrop-blur-xl rounded-2xl p-5 sm:p-6 border border-purple-500/30 hover:border-purple-500/50 transition-all">
                    ...
                    <p className="text-3xl sm:text-4xl font-bold text-white mb-1">{stats.total}</p>
                    <p className="text-xs sm:text-sm text-purple-300">All Tasks</p>
                  </div>
                </div>
                ...
                <div className="relative group">
                  ...
                  <div className="relative bg-slate-900/60 backdrop-blur-xl rounded-2xl p-5 sm:p-6 border border-emerald-500/30 hover:border-emerald-500/50 transition-all">
                    <div className="flex items-start justify-between mb-3">
                      ...
                      <span className="text-xs font-medium text-emerald-400 bg-emerald-500/10 px-2 py-1 rounded-full">{completionRate}%</span>
                    </div>
                    <p className="text-3xl sm:text-4xl font-bold text-white mb-1">{stats.completed}</p>
                    <p className="text-xs sm:text-sm text-emerald-300">Completed</p>
                  </div>
                </div>
                ...
                <div className="relative group">
                  ...
                  <div className="relative bg-slate-900/60 backdrop-blur-xl rounded-2xl p-5 sm:p-6 border border-amber-500/30 hover:border-amber-500/50 transition-all">
                    <div className="flex items-start justify-between mb-3">
                      ...
                      <span className="text-xs font-medium text-amber-400 bg-amber-500/10 px-2 py-1 rounded-full">Active</span>
                    </div>
                    <p className="text-3xl sm:text-4xl font-bold text-white mb-1">{stats.pending}</p>
                    <p className="text-xs sm:text-sm text-amber-300">In Progress</p>
                  </div>
                </div>
              </div>
                ...
                {filteredTasks.length === 0 ? (
                  <div className="relative group">
                    ...
                    <div className="relative bg-slate-900/60 backdrop-blur-xl rounded-2xl p-12 sm:p-16 text-center border border-white/10">
                      ...
                      <h3 className="text-lg sm:text-xl font-bold text-white mb-2">No tasks found</h3>
                    ...
                    >
                      ...
                      <div className="relative bg-slate-900/60 backdrop-blur-xl rounded-2xl p-4 sm:p-5 border border-white/10 hover:border-purple-500/30 transition-all">
                        <div className="flex items-start gap-3 sm:gap-4">
                          ...
                          <div className="flex-1 min-w-0">
                            <div className="flex flex-wrap items-start justify-between gap-2 mb-2">
                              <h3 className={`text-base sm:text-lg font-semibold break-words ${task.status === "Completed" ? 'text-slate-500 line-through' : 'text-white'}`}>
                                {task.title}
                              </h3>
                              <span className={`px-2.5 py-1 rounded-lg text-xs font-semibold border flex-shrink-0 ${getPriorityBadge(task.priority)}`}>
                                {task.priority}
                              </span>
                            </div>

                            {task.description && (
                              <p className={`text-xs sm:text-sm mb-3 break-words leading-relaxed ${task.status === "Completed" ? 'text-slate-500' : 'text-slate-400'}`}>
                                {task.description}
                              </p>
                            )}
                        ...
                        <div className="mt-4 h-1.5 rounded-full bg-slate-800/50 overflow-hidden">
                          <div 
                            className={`h-full bg-gradient-to-r ${getPriorityColor(task.priority)} ${
                              task.status === "Completed" ? 'w-full' : 'w-1/3'
                            } transition-all duration-700 shadow-lg`}
                            style={{
                              boxShadow: task.status === "Completed" ? '0 0 10px currentColor' : 'none'
                            }}
                          ></div>
                        </div>
                      </div>
